...
function generic_run(sample::SampleStruct;
    ...
    D, W_ba, tau, alpha = checks_before_run(sample.n_resources, D, W_ba, tau, alpha)
...
# invaders appear in first section
function spatial_run(n_comms::Int64, sample::SampleStruct;
    ...
    for i in 1:n_comms
        ...
        se_dict[i] = se
        if plot
            plot_se(se, "sim", path + "community$i/")
            plot_MCI(se, "sim", path + "community$i/")
        end
    end
